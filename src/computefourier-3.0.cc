...
void compute_bucketed_signal(int n, int d, int lvl, const Key& sigma, const Key& b, const Key& a, const FilterCompact& filter, Key& u_index, Key& in_index, complex_t value, const complex_t* in, complex_t* u) {
    if (lvl < d) {
        for (int h = 0; h < filter.sizet; ++h) {
            ...
            double phi = -2 * M_PI / n * (((((int64_t) sigma.at(lvl)) * b.at(lvl)) * cur_ind) & (n - 1));
...
//        assert(abs(creal(u[u_index.flatten()])) < 100);
    }
}

void hash_to_bins(int n, int d, int Btotal, complex_t* in, const Key& sigma, const Key& b, const Key& a, const sfft_output& out, const FilterCompact& filter, complex_t* u, const fftw_plan& p) {
...
//  int period = n / filter.B_g;
  ...
  for (__typeof(out.begin()) it = out.begin(); it != out.end(); ++it) {
    ...
    int pos = 0;
    for (int i = d - 1; i >= 0; --i) {
      double phi = -2 * M_PI / n * ((((int64_t) sigma.at(i)) * a.at(i) * index.at(i)) & (n - 1));
      value *= (cos(phi) + I * sin(phi));
      index.at(i) = (((int64_t) sigma.at(i)) * ((index.at(i) - b.at(i) + n) & (n - 1))) & (n - 1);
      int64_t j = ((index.at(i) + (n / filter.B_g) / 2) & (n - 1)) / (n / filter.B_g);
      value *= filter.freq_at((j * (n / filter.B_g) - index.at(i) + n) & (n - 1));
